...
def load_assets():
    try:
        # Metadata'yı yükle (3-Tier için güncellenmiş metadata olmalı)
        meta = joblib.load("model_metadata_3tier.pkl")

        # Standard Modeli Yükle (Zorunlu)
        m_std = CatBoostRegressor()
        m_std.load_model("catboost_standard.cbm")

        # Super Modeli Yükle (Opsiyonel)
        m_super = None
        try:
            temp_super = CatBoostRegressor()
            temp_super.load_model("catboost_super.cbm")
            m_super = temp_super
        except:
            pass  # Dosya yoksa m_super None kalır

        return m_std, m_super, meta
    except Exception as e:
        st.error(
            f" 'model_metadata_3tier.pkl' veya 'catboost_standard.cbm' bulunamadı.\nHata: {e}")
